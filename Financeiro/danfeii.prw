...
#DEFINE MAXITEMC   023 //038                                                // M�xima de caracteres por linha de produtos/servi�os
#DEFINE MAXMENLIN  090 //080                                                // M�ximo de caracteres por linha de dados adicionais
...
User Function PrtNfeSef(cIdEnt,cVal1,cVal2,oDanfe,oSetup,cFilePrint,l_Automati)
...
RptStatus({|lEnd| DanfeProc(@oDanfe,@lEnd,cIdEnt,,,@lExistNfe,l_Automati)},"Imprimindo Danfe...")
...
Static Function DanfeProc(oDanfe,lEnd,cIdEnt,cVal1,cVal2,lExistNfe,l_Automati)
				...
				For nX := 1 To nLenNotas
					If !Empty(aXML[nX][2])
						...
						If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5,7") .And. !cCodRetNFE $ RetCodDene()
							...
							// FUNCAO ESPECIFICA PARA IMPRESS�O DA GUIA DE GNRE - MONTES - WIKITEC - 21/04/2019
...
For nZ := 1 To nLenDet
	...
	nMaxCod := MaxCod(oDet[nX]:_Prod:_cProd:TEXT, 60) //, 50)  //Montes - Oficina1 em 16/12/2011
	nMaxDes := MaxCod(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte), 146) //145) //nMaxDes := 30//MaxCod(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte), 100)  //Montes - Oficina1 em 16/12/2011
	...
	While !Empty(cAux) .Or. !Empty(cAuxItem) .Or. !Empty(aAux[1]) .Or. !Empty(aAux[2]) .Or. !Empty(aAux[3]) .Or. !Empty(aAux[4]) .Or. !Empty(aAux[5]) .Or. !Empty(aAux[6])
		nMaxCod := MaxCod(cAuxItem, 60)   //, 50) //Montes - Oficina1 em 16/12/2011
		nMaxDes := MaxCod(cAux, 145) //, 100) //Montes - Oficina1 em 16/12/2011
...
// Joyce: Inclui Regi�o de entrega do cliente nos dados complementares - 25/09/2018
SA1->(dbSetOrder(1))
If SA1->(MsSeek(xFilial("SA1")+ SF2->F2_CLIENT + SF2->F2_LOJA)) .And. !Empty(SA1->A1_XREGCLI)
	cRegCli := Posicione("SX5",1,xFilial("SX5")+"Z2"+SA1->A1_XREGCLI,"X5_DESCRI")
EndIf

If !Empty(cRegCli)
	cAux := "Regi�o do Cliente: "+cRegCli
	While !Empty(cAux)
		aadd(aMensagem,SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN) - 1, MAXMENLIN)))
		cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN), MAXMENLIN) + 1)
	EndDo
...
//������������������������������������������������������������������������Ŀ
//�Inclus�o do logotipo                                                    �
//��������������������������������������������������������������������������
//Incluido por  Ricardo Alberti - Oficina1 em 29/03/11
If cEmpAnt = '01'
	If File("\system\logonfe_ellfas.bmp")
		cLogo := "\system\logonfe_ellfas.bmp"
	EndIf
ElseIf cEmpAnt = '02'
	If File("\system\logonfe_arl.bmp")
		cLogo := "\system\logonfe_arl.bmp"
	EndIf
ElseIf cEmpAnt = '03'
	If File("\system\logonfe_marlin.bmp")
		cLogo := "\system\logonfe_marlin.bmp"
	EndIf
EndIf	
...
//��������������������������������������������������������������������������
If nfolha==1
	oDanfe:SayBitmap(050,001,cLogo,224,082)    
Endif
/*
If lMv_Logod
	...
	If lflag
		If nL > nMaxItemP2
			...
			*/
			...
			EndIf
			*/
			oDanfe:SayBitmap(001,001,cLogo,224,082)    
...
//�Inclus�o do logotipo                                                    �
//��������������������������������������������������������������������������
//Incluido por  Ricardo Alberti - Oficina1 em 21/03/11
If cEmpAnt = '01'
	If File("\system\logonfe_ellfas.bmp")
		cLogo := "\system\logonfe_ellfas.bmp"
	EndIf
...
oDanfe:Say(130,258,"S�RIE "+oIdent:_Serie:Text,oFont10N:oFont)
   ...
   If dbSeek(cChaveSF6)                       
	  While SF6->(!EOF()) .And. SF6->(F6_FILIAL+F6_OPERNF+F6_TIPODOC+F6_DOC+F6_SERIE+F6_CLIFOR+F6_LOJA) == cChaveSF6
		 ...
		 If SF6->F6_TIPOIMP $ "3-B"  //ICMS ST e DIFAL #FECP DIFAL            
			...
			Else
			   Aviso("GNRE","GNRE de "+cUF+" n�o autorizada! Verifique mensagem na monitor de transmissao de GNRE.",{"Ok"},3)
			EndIf
		 EndIf   
